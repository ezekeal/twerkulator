...
export default function fish(accelData, htmlNode: HTMLElement) {
  new p5instance((p5) => {
    preload(p5);
    //p5.smooth();
    // p5.frameRate(25);
    ...
    class Sprite {
      private r;
      private x = p5.random(p5.width);
      private y = p5.random(p5.height);
      private xspeed = p5.map(this.r, bgsize, frsize, 0.5, 2, true); //map the speed in x-direction based on the size/layer of the sprites
      private diameter = p5.random(10, 30);
      private speed = 1;
      private bcolor = p5.color(255, 204, 10);
      constructor(tempR) {
        this.r = tempR;
      ...
      move(scaledAccelPoint) {
        this.x +=
          this.xspeed * p5.map(scaledAccelPoint.x, -400, 400, -5, 5, true); // Increment x
        this.y +=
          this.xspeed * p5.map(scaledAccelPoint.y, -200, 200, -5, 5, true);
        // Check edges
      ...
      display(scaledAccelPoint) {
        ...
        let sizeFactor = p5.map(
          (scaledAccelPoint.z + scaledAccelPoint.x) / 2,
          -400,
          400,
          0.8,
          1.2,
    ...
    p5.setup = () => {
      p5.frameRate(60);
      p5.createCanvas(htmlNode.clientWidth, htmlNode.clientHeight);
    };

    // accelData.start();
    //return ({ p5, time, width, height, frame }) =>
