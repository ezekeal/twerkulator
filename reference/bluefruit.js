...
let panels = {
    battery: {
        ...
        update: function (panelId) {
            let panelElement = document.querySelector(
                '#dashboard > #' + panelId
            );
            let value = null;
            ...
            if (value != null && value <= 25) {
                // Show Red
                panelElement
                    .querySelector('.content .battery')
                    .classList.remove('battery-middle');
                panelElement
                    .querySelector('.content .battery')
                    .classList.add('battery-alert');
            } else if (value == null || value <= 50) {
                // Show Yellow
                panelElement
                    .querySelector('.content .battery')
                    .classList.remove('battery-alert');
                panelElement
                    .querySelector('.content .battery')
                    .classList.add('battery-middle');
            } else {
                // Show Green
                panelElement
                    .querySelector('.content .battery')
            ...
            if (value == null) {
                ...
                panelElement.querySelector('.content .battery').title =
                    'Battery Level: ?';
            } else {
                panelElement.querySelector(
                    '.content .battery .level'
    ...
    buttons: {
        ...
        create: function (panelId) {
            ...
            for (let i = 0; i < currentBoard.buttons; i++) {
                let buttonTemplate = document
                    .querySelector('#templates > .roundbutton')
                    .cloneNode(true);
                buttonTemplate.id = 'button_' + (i + 1);
                buttonTemplate.querySelector('.text').innerHTML =
        ...
        update: function (panelId) {
            ...
            for (let i = 1; i <= currentBoard.buttons; i++) {
                if (buttonState & (1 << i)) {
                    panelElement
                        .querySelector('#button_' + i + ' .roundbtn')
    ...
    switch: {
        ...
        update: function (panelId) {
            ...
            panelElement.querySelector('.content #onboardSwitch').checked =
                buttonState & 1;
        },
    },
    humidity: {
    ...
    tone: {
        ...
        create: function (panelId) {
            ...
            panelTemplate.querySelector('.content .button').onclick =
                function () {
                    let button = this;
                    button.disabled = true;
                    playSound(440, 1000, function () {
...
function encodePacket(panelId, values) {
    ...
    if (values.length != panels[panelId].packetSequence.length) {
        logMsg(
            'Error in encodePacket(): Number of arguments must match structure'
        );
        return false;
    }
...
async function connect() {
    ...
    if (device) {
        ...
        for (let panelId of Object.keys(panels)) {
            if (
                panels[panelId].condition == undefined ||
                panels[panelId].condition()
            ) {
                if (
                    getFullId(panels[panelId].serviceId).substr(0, 4) == 'adaf'
                ) {
                    for (const service of availableServices) {
                        if (
                            getFullId(panels[panelId].serviceId) == service.uuid
                        ) {
                            createPanel(panelId);
                        }
                    }
        ...
        for (let panelId of activePanels) {
            let service = await server
                .getPrimaryService(getFullId(panels[panelId].serviceId))
                .catch((error) => {
                    console.log(error);
                });
            if (service) {
                panels[panelId].characteristic = await service
                    .getCharacteristic(
                        getFullId(panels[panelId].characteristicId)
                    )
                    .catch((error) => {
                        console.log(error);
                    });
                ...
                logMsg(
                    '> Characteristic UUID:  ' +
                        panels[panelId].characteristic.uuid
                );
                logMsg(
                    '> Broadcast:            ' +
                        panels[panelId].characteristic.properties.broadcast
                );
                logMsg(
                    '> Read:                 ' +
                        panels[panelId].characteristic.properties.read
                );
                logMsg(
                    '> Write w/o response:   ' +
                        panels[panelId].characteristic.properties
                            .writeWithoutResponse
                );
                logMsg(
                    '> Write:                ' +
                        panels[panelId].characteristic.properties.write
                );
                logMsg(
                    '> Notify:               ' +
                        panels[panelId].characteristic.properties.notify
                );
                logMsg(
                    '> Indicate:             ' +
                        panels[panelId].characteristic.properties.indicate
                );
                logMsg(
                    '> Signed Write:         ' +
                        panels[panelId].characteristic.properties
                ...
                logMsg(
                    '> Queued Write:         ' +
                        panels[panelId].characteristic.properties.reliableWrite
                );
                logMsg(
                ...
                if (panels[panelId].properties.includes('notify')) {
                    if (panels[panelId].measurementPeriod !== undefined) {
                        let mpChar = await service
                            .getCharacteristic(getFullId(measurementPeriodId))
                            .catch((error) => {
                                console.log(error);
                            });
                        ...
                        view.setInt32(
                            0,
                            panels[panelId].measurementPeriod,
                            true
                        );
                        mpChar
                            .writeValue(view.buffer)
                            .catch((error) => {
                                console.log(error);
                            })
                            .then((_) => {
                                logMsg(
                                    'Changed measurement period for ' +
                                        ucWords(panelId) +
                                        ' to ' +
                                        panels[panelId].measurementPeriod +
                                        'ms'
                    ...
                    panels[panelId].characteristic.addEventListener(
                        'characteristicvaluechanged',
                        function (event) {
                            handleIncoming(panelId, event.target.value);
                        }
                    );
                }
...
async function readActiveSensors() {
    for (let panelId of activePanels) {
        ...
        if (
            panels[panelId].properties.includes('read') ||
            panels[panelId].properties.includes('notify')
        ) {
            await panels[panelId].characteristic
                .readValue()
                .then(function (data) {
                    handleIncoming(panelId, data);
                });
...
async function reset() {
    ...
    for (let panelId of activePanels) {
        ...
        if (panels[panelId].data !== undefined) {
            Object.entries(panels[panelId].data).forEach(
                ([field, item], index) => {
...
function loadPanelTemplate(panelId, templateId) {
    ...
    let panelTemplate = document
        .querySelector('#templates > .' + templateId)
        .cloneNode(true);
...
function createGraphPanel(panelId) {
    ...
    // Setup graph
    Object.entries(panels[panelId].data).forEach(([field, item], index) => {
        panels[panelId].graph.addDataSet(
            field,
...
function createColorPanel(panelId) {
    ...
    function updateModelLed(color) {
        ...
        let orderedColors = adjustColorOrder(
            Math.round(color.r() * 255),
            Math.round(color.g() * 255),
            Math.round(color.b() * 255)
        );
        let values = [0, 1].concat(
            new Array(currentBoard.neopixels).fill(orderedColors).flat()
        );
...
function create3dPanel(panelId) {
    ...
    panels[panelId].camera = new THREE.PerspectiveCamera(
        45,
...
function update3dPanel(panelId) {
    ...
    panels[panelId].renderer.render(
        panels[panelId].scene,
        panels[panelId].camera
    );
}
